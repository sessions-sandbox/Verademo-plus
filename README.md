...
```
docker run --rm -it -p 127.0.0.1:8080:8080 --entrypoint bash -v "$(pwd):/app" verademo
```
...
Fix for SCA is enabled on this repository, so when creating a PR, you can interact with the /veracode-fix bot to fix items.
